Shader "Infinite Runner/Diffuse CurveNormal2Heller" {
	Properties {
	...
	SubShader {
        ...
        Pass { // pass 0
			...
			uniform float4 _FadeOutColorFirst;
			...
			fragmentInput vert(appdata_full v)
			{
				...
				// Apply the curve
                float4 pos = mul(UNITY_MATRIX_MV, v.vertex); //o.vertex = UnityObjectToClipPos(v.vertex);
			...
			{
				fixed4 color = tex2D(_MainTex, i.uv) *_FadeOutColorFirst; //Farbe dynamisch ändern mit Parameter
