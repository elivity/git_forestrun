...
Shader "Infinite Runner/Diffuse Curve" {
	Properties {
		_MainTex ("Main Texture", 2D) = "black" {} //öffentlich sichtbare Variablen (TextureMap, Regler usw.) 
		_FadeOutColor ("Fade Out Color", Color) = (0, 0, 0, 0)
		_NearCurve ("Near Curve", Vector) = (0, 0, 0, 0)
		_FarCurve ("Far Curve", Vector) = (0, 0, 0, 0)
		_Dist ("Distance Mod", Float) = 0.0 //positive Zahl-> rechts Krümmung, negativ -> links Krümmung
	}
	SubShader {
		Tags { "RenderType"="Opaque" } //opaque = undurchlässig/lichtundurchlässig
        ...
        Pass { // pass 0
			...
			fragmentInput vert(appdata_full v)
			{
                ...
                o.distanceSquared = pos.z * pos.z * _Dist;// o.distanceSquared = pos.z * pos.z * _Dist;
                pos.x += (_NearCurve.x - max(1.0 - o.distanceSquared / _FarCurve.x, 0.0) * _NearCurve.x); //curve an X-achse nach rechts/links biegen
